...
class GaussianModel:
    ...
    def save_rgb_ply(self, path):
        ...
        elements = np.empty(xyz.shape[0], dtype=dtype_full)
        attributes = np.concatenate((xyz, rgb, opacity), axis=1)
        elements[:] = list(map(tuple, attributes))
        el = PlyElement.describe(elements, 'vertex')
        PlyData([el]).write(path)
    ...
    def scale_adjustment(self, scale_factor):
        scales_new = self.scaling_inverse_activation(self.get_scaling * scale_factor)
